...
if docker info >/dev/null 2>&1; then
	...
	docker run -it --name=/DroidAugmentor-$RANDOM -v $DIR:/DroidAugmentor/shared -e DISPLAY=unix$DISPLAY  wrseg/droidaugmento:latest pipenv run python3 run_campaign.py
	#ls shared/outputs/
else
